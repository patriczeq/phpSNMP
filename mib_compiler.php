 ...
 * 
 * EDIT 2021 Patrik Eder <patrik@eders.cz>
 *
 ...
 */
  class mib_compiler
  {
    ...
    */
    public function add_mib($filename, $outfile = null)
    {
      if($outfile !== null)
        {
      ...
      $this->_echo("Loading $filename\n");
      
      $file_content = file_get_contents($filename);
      $imports = $this->get_string_between($file_content, 'IMPORTS', ';');
      if($imports !== '')
        {
          foreach(explode('FROM ', $imports) as $row)
            {
              $mib = str_replace(array("\n", "\r", " "), array("", "", ""), explode("\n", $row)[0]);
              $mib = $mib === "" ? false : $mib;
              if($mib)
                {
                  $importFname = dirname($filename) . "/" . $mib;
                  if(file_exists($importFname))
                    {
                      $this->add_mib($importFname);
                    }
                  else if(file_exists($importFname . '.mib'))
                    {
                      $this->add_mib($importFname . '.mib');
                    }
                  else if(file_exists($importFname . '.txt'))
                    {
                      $this->add_mib($importFname . '.txt');
                    }
                  else
                    {
                      $importFname = $this->mibFileSearch(dirname($filename) . "/", $mib);
                      if($importFname !== null)
                        {
                          $this->add_mib($importFname);
                        }
    ...
    */
    ...
    {
      $this->_echo('Found ' . count($this->parsed) . " objects\n");
      $this->_echo("Building node list\n");
      ...
      $this->_echo("Trimming disconnected nodes\n");
      foreach(array_keys($nodes) as $key)
      {
        if(!isset($found[$key]))
        {
          $this->_echo("deleting $key\n");
          unset($nodes[$key]);
        }
      }

      $this->_echo("Writing {$this->outfile}\n");
    ...
    */
    public function parse($mibtext, $full=false)
    {
      ...
      $cnt = count($tokens);
      $this->_echo("Found $cnt tokens\n");
      $rec = array();
      // fix IMPORTS
      
      for($index = 0; $index < $cnt; $index++)
      {
        $this->_echo(number_format(100 *$index / $cnt, 2) . "%                                \r");
      ...
      $this->_echo(number_format(100, 2) . "%                                \r");
      if(isset($rec['NAME']) && isset($rec['VALUE'])) $this->parsed[] = $rec;
    }
